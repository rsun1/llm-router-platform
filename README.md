...
### Compress 使用 Mistral 而非 Qwen2.5
压缩节点最初用本地 Qwen2.5-0.5B 实现。但是**实测它无法完成任务**,会误判"无需压缩"并转而回答问题。换 mistral-7b 后正常。这和分类器的结论形成一正一反两个例证:
输出一个 token 且经过微调的分类任务,0.5B 足够;理解长文并改写的压缩任务,0.5B 不够。
### fallback 的三个设计决策
| 原待决 | 结论 | 理由 |
|---|---|---|
